...
class ResumeUploadDataSource {
  ...
  Future<ApiResponse> uploadResumeBytes(Uint8List bytes, String fileName) async {
    try {
      log('Uploading resume bytes: $fileName');
      
      final response = await _client.uploadBytes(Constants.uploadResume, bytes, fileName, 'file');
