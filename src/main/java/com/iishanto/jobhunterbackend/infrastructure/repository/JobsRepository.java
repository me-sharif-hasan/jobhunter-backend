...
public interface JobsRepository extends JpaRepository<Jobs, String> {
    @Query(value = "SELECT * FROM jobs,site WHERE jobs.is_duplicate = false AND (site.id = site_id AND site_id IN :siteIds AND (title LIKE %:keyword% OR job_description LIKE %:keyword% OR site.name LIKE %:keyword%)) ORDER BY job_parsed_at DESC", nativeQuery = true)
